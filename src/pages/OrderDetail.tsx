...
фimport { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { subscribeToOrder, request } from '../lib/api';
...
const STATUS_ICON_NAMES: Record<string, 'pending' | 'confirmed' | 'collecting' | 'ready' | 'delivering' | 'delivered' | 'cancelled'> = {
  created: 'pending',
  pending: 'pending',
  confirmed: 'confirmed',
  collecting: 'collecting',
  ...
  if (!order) {
    return (
      <div className={styles.page}>
        ...
        <div className="empty-state"><h3>Заказ не найден</h3></div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <span className={styles.headerTitle}>{(order as any).order_number || `#${order.id.slice(-6).toUpperCase()}`}</span>
        <div style={{ width: 40 }} />
      </div>
      ...
      <div className={`scroll-area ${styles.content}`}>
        ...
        <div className={`card ${styles.statusCard}`}>
          <div className={styles.statusIcon}><Icon name={STATUS_ICON_NAMES[order.status]} size={48} /></div>
          <h2 className={styles.statusLabel}>{ORDER_STATUS_LABELS[order.status]}</h2>
          {isActive && (
            ...
            <div className={styles.progress}>
              {ORDER_STATUS_STEPS.map((step, idx) => (
                <div key={step} className={styles.progressStep}>
                  <div className={`${styles.progressDot} ${idx <= currentStep ? styles.activeDot : ''}`}>
                    {idx < currentStep ? '✓' : idx === currentStep ? '●' : ''}
                  </div>
                  {idx < ORDER_STATUS_STEPS.length - 1 && (
                    <div className={`${styles.progressLine} ${idx < currentStep ? styles.activeLine : ''}`} />
                  )}
                </div>
              ))}
            </div>
          )}
          <p className={styles.statusDate}>{formatDate(order.updated_at || order.created_at)}</p>
        </div>
              ...
              {order.items.map((item: any) => (
                <div key={item.id} className={styles.item}>
                  ...
                  <div className={styles.itemInfo}>
                    <p className={styles.itemName}>{item.medicine?.name || item.medicine_name || 'Лекарство'}</p>
                    <p className={styles.itemQty}>{item.quantity} шт × {formatPrice(item.unit_price || item.price)}</p>
                  </div>
                  <span className={styles.itemTotal}>{formatPrice((item.unit_price || item.price) * item.quantity)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {order.pharmacy && (
          <div className={`card ${styles.section}`}>
            ...
            {order.pharmacy.phone && (
              <a href={`tel:${order.pharmacy.phone}`} className={styles.pharmPhone}>
                📞 {order.pharmacy.phone}
              </a>
            )}
          </div>
