  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <span className={styles.headerTitle}>Заказ #{order.id.slice(-6).toUpperCase()}</span>
        <div style={{ width: 40 }} />
      </div>
              ...
              {order.items.map((item) => (
                <div key={item.id} className={styles.item}>
                  ...
                  <div className={styles.itemInfo}>
                    <p className={styles.itemName}>{item.medicine?.name || 'Лекарство'}</p>
                    <p className={styles.itemQty}>{item.quantity} шт × {formatPrice(item.price)}</p>
                  </div>
