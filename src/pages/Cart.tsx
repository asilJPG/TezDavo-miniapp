...
export function CartPage() {
  ...
  const { items, removeItem, updateQuantity, clear, total, count } = useCartStore();
  ...
  const needsPrescription = items.some((i) => i.medicine.prescription_required);
  const pharmacy = items[0]?.inventory.pharmacy;

  async function handleOrder() {
    if (!user) { toast.error('Войдите через Telegram'); return; }
    ...
    return (
      <div className={styles.page}>
        ...
        <div className="empty-state">
          ...
          <button className="btn btn-primary" style={{ marginTop: 8, width: 'auto', padding: '12px 24px' }} onClick={() => navigate('/search')}>
            Найти лекарства
          </button>
        </div>
      </div>
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>Корзина</h2>
        <button className="btn btn-ghost" style={{ color: 'var(--red)', fontSize: 13 }} onClick={() => { if (confirm('Очистить корзину?')) clear(); }}>
          Очистить
        </button>
      </div>
      ...
      <div className={`scroll-area ${styles.content}`}>
          ...
          {items.map((item) => (
            <div key={item.inventory.id} className={`card ${styles.cartItem}`}>
              ...
              <div className={styles.itemInfo}>
                ...
                <p className={styles.itemPrice}>{formatPrice(item.inventory.price)} / шт</p>
              </div>
              <div className={styles.quantityControl}>
                <button className={styles.qBtn} onClick={() => updateQuantity(item.inventory.id, item.quantity - 1)}>−</button>
          ...
          {needsPrescription && (
            <div className={styles.field}>
              <label className={styles.label}>
                Фото рецепта *
                <span className="badge badge-yellow" style={{ marginLeft: 6 }}>Обязательно</span>
              </label>
              <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={(e) => setPrescriptionFile(e.target.files?.[0] || null)} />
              <button className={`btn btn-outline ${styles.uploadBtn}`} onClick={() => fileRef.current?.click()}>
                {prescriptionFile ? (
        ...
        <div className={`card ${styles.summary}`}>
          ...
          <div className={styles.summaryRow} style={{ color: 'var(--gray-500)', fontSize: 12 }}>
            <span>Доставка</span>
            <span>Бесплатно</span>
          </div>
        </div>
        ...
        <div style={{ paddingBottom: 16 }}>
          <button className="btn btn-primary" onClick={handleOrder} disabled={loading}>
            {loading ? <span className="spinner" style={{ width: 18, height: 18, borderTopColor: 'white' }} /> : `Заказать • ${formatPrice(total())}`}
          </button>
        </div>
      </div>
    </div>
