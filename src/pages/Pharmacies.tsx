      ...
      {view === 'map' ? (
        <div className={styles.mapPlaceholder}>
          <div className={styles.mapInner}>
            ...
            <h3>Карта аптек</h3>
            <p>Подключите Yandex Maps или Google Maps API для отображения карты</p>
          ...
          <div className={styles.pins}>
            {pharmacies.filter(p => p.latitude).map(p => (
              <Link key={p.id} to={`/pharmacy/${p.id}`} className={styles.pin}>
                🏥 {p.name}
              </Link>
            ))}
          </div>
        ...
        <div className={`scroll-area ${styles.content}`}>
          {loading ? (
            ...
            pharmacies.map((ph) => (
              <Link key={ph.id} to={`/pharmacy/${ph.id}`} className={`card ${styles.card} fade-in`}>
                <div className={styles.logo}>🏥</div>
                <div className={styles.info}>
                  <p className={styles.name}>{ph.name}</p>
                  <p className={styles.address}>{ph.address}</p>
                  {ph.working_hours && <p className={styles.hours}>{ph.working_hours}</p>}
                </div>
                <div className={styles.right}>
                  <span className={`badge ${ph.is_active ? 'badge-green' : 'badge-gray'}`}>
                    {ph.is_active ? 'Открыта' : 'Закрыта'}
                  </span>
                  {ph.rating && <p className={styles.rating}>⭐ {ph.rating.toFixed(1)}</p>}
                </div>
              </Link>
            ))
          )}
