...
export function ProfilePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setEditing(false);
      toast.success('Профиль обновлён', { style: { borderRadius: '12px', fontFamily: 'Onest' } });
    } catch (e: any) {
  ...
  const menuItems = [
    { icon: '📦', label: 'Мои заказы', path: '/orders' },
    { icon: '💊', label: 'График приёма', path: '/schedule' },
    { icon: '🏥', label: 'Аптеки', path: '/pharmacies' },
  ];
      ...
      <div className={`scroll-area ${styles.content}`}>
        ...
        <div className={`card ${styles.profileCard}`}>
          ...
          {!editing ? (
            <>
              ...
              {user?.phone && <p className={styles.phone}>{formatPhone(user.phone)}</p>}
              <p className={styles.tgId}>Telegram ID: {user?.telegram_id}</p>
              <button className="btn btn-outline" style={{ marginTop: 12, padding: '10px 24px', width: 'auto' }} onClick={() => setEditing(true)}>
                Редактировать
              </button>
            </>
          ) : (
            <div className={styles.editForm}>
              ...
              <div className={styles.editBtns}>
                ...
                <button className="btn btn-primary" style={{ flex: 1 }} onClick={handleSave} disabled={saving}>
                  {saving ? <span className="spinner" style={{ width: 16, height: 16, borderTopColor: 'white' }} /> : 'Сохранить'}
                </button>
              </div>
            </div>
          )}
        ...
        <div className={`card ${styles.menu}`}>
          {menuItems.map((item, i) => (
            <button key={item.path} className={styles.menuItem} onClick={() => navigate(item.path)}>
              <span className={styles.menuIcon}>{item.icon}</span>
              <span className={styles.menuLabel}>{item.label}</span>
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
                <path d="M9 18l6-6-6-6" stroke="var(--gray-400)" strokeWidth="2" strokeLinecap="round" />
              </svg>
              {i < menuItems.length - 1 && <hr className="divider" style={{ position: 'absolute', bottom: 0, left: 54, right: 0 }} />}
            </button>
          ))}
        </div>
        ...
        <p className={styles.footerNote}>
          Доставка лекарств по Ташкенту 24/7.<br />
