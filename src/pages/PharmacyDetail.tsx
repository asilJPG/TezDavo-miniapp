...
export function PharmacyDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    pharmaciesApi.getById(id).then((data: any) => {
  ...
  const filtered = inventory.filter((i) =>
    i.medicine?.name?.toLowerCase().includes(search.toLowerCase()) && i.in_stock
  );
  ...
  return (
    <div className={styles.page}>
      ...
      {loading ? (
        ...
        <div className="empty-state"><h3>Аптека не найдена</h3></div>
      ) : (
        <div className={`scroll-area ${styles.content}`}>
          ...
          <div className={`card ${styles.hero}`}>
            ...
            {pharmacy.working_hours && (
              <p className={styles.hours}>🕐 {pharmacy.working_hours}</p>
            )}
            <div className={styles.tags}>
              <span className={`badge ${pharmacy.is_active ? 'badge-green' : 'badge-gray'}`}>
                {pharmacy.is_active ? 'Открыта' : 'Закрыта'}
              </span>
              {pharmacy.phone && (
                <a href={`tel:${pharmacy.phone}`} className={styles.phone}>📞 {pharmacy.phone}</a>
              )}
            </div>
          </div>
          ...
          <div className={styles.section}>
            ...
            {filtered.length === 0 ? (
              ...
              <div className={styles.inventoryList}>
                {filtered.map((item) => (
                  <Link key={item.id} to={`/medicine/${item.medicine_id}`} className={`card ${styles.invItem}`}>
                    <div className={styles.invIcon}>💊</div>
                    <div className={styles.invInfo}>
                      <p className={styles.invName}>{item.medicine?.name}</p>
                      {item.medicine?.manufacturer && (
                        <p className={styles.invMfr}>{item.medicine.manufacturer}</p>
                      )}
                    </div>
                    <div className={styles.invRight}>
                      <p className={styles.invPrice}>{formatPrice(item.price)}</p>
